...
// };
const getLineClient = (isAdmin = false) => {
  const config = {
    channelAccessToken: isAdmin
      ? process.env.ADMIN_CHANNEL_ACCESS_TOKEN
      : process.env.CLIENT_CHANNEL_ACCESS_TOKEN,
    channelSecret: isAdmin
      ? process.env.ADMIN_CHANNEL_SECRET
      : process.env.CLIENT_CHANNEL_SECRET,
  };
