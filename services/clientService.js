...
export const handleEvent = async (event) => {
    ...
    if (localOtp && !otp.find((otps) => otps.otp === localOtp.otp)) {
        ...
        return replyText(event.replyToken, "OTP หมดอายุแล้ว\nโปรดส่งรหัส OTP ใหม่", event.message.quoteToken);
    }

    if (globalOtp.find((otps) => otps.otp === event.message.text)) {
        fileStorage.writeStorage({ otp: event.message.text });
        return replyText(event.replyToken, "OTP ของคุณถูกต้อง", event.message.quoteToken);
    }
...
const handleMessage = (message, replyToken) => {
    switch (message.type) {
        ...
        case "location":
        case "sticker":
            return replyText(replyToken, `Got ${message.type.charAt(0).toUpperCase() + message.type.slice(1)}`);
        default:
